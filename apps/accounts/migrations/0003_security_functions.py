...
REVOKE ALL ON FUNCTION cipa_has_tenant_access(uuid, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION cipa_invitation_tenant(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION cipa_is_platform_admin(uuid) FROM PUBLIC;
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'cipa_app') THEN
    GRANT EXECUTE ON FUNCTION cipa_has_tenant_access(uuid, uuid) TO cipa_app;
    GRANT EXECUTE ON FUNCTION cipa_invitation_tenant(text) TO cipa_app;
    GRANT EXECUTE ON FUNCTION cipa_is_platform_admin(uuid) TO cipa_app;
  END IF;
END
$$;
"""
...
REVERSE_SQL = """
DROP FUNCTION IF EXISTS cipa_invitation_tenant(text);
DROP FUNCTION IF EXISTS cipa_has_tenant_access(uuid, uuid);
DROP FUNCTION IF EXISTS cipa_is_platform_admin(uuid);
"""
